...
import { getCache, CACHE_TTL, setCache } from '@/lib/redis';

const TEAM_CACHE_KEY = 'team_members_v2';

// Mirrors the full OFFICIAL_ROSTER from the team route
...
export async function GET(
  ...
  try {
    ...
    // Try to pull from cache first (full roster)
    const cached = await getCache(TEAM_CACHE_KEY) as any[] | null;
    const roster: any[] = Array.isArray(cached) ? cached : OFFICIAL_ROSTER;

    const player = roster.find((p) => p.id === numId);
    if (!player) {
