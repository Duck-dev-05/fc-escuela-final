...
export default function OperationDeepDive() {
   ...
   const getFormationType = (lineup: any[]) => {
      ...
      if (assigned === 0) return 'DEPLOYMENT_PENDING';
      if (assigned < 7) return `INCOMPLETE_GRID (${assigned}/7)`;
      return '2-3-1 VANGUARD_SERIES';
   };
   ...
   useEffect(() => {
      async function fetchData() {
         ...
         try {
            ...
            console.error('REGISTRY_ACCESS_FAILED:', e);
         } finally {
            setLoading(false);
         }
      }
   ...
   const handleSlotClick = (slot: any) => {
      ...
      if (assigned) {
         // Find player details in team array to show full info
   ...
   const saveOperation = async () => {
      setSaving(true);
      try {
         // Calculate final power indices before saving
         ...
         if (res.ok) {
            alert('TACTICAL_DEPLOYMENT_AUTHORIZED');
         }
      } catch (e) {
         console.error(e);
      } finally {
   ...
   const handlePitchMove = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      setTilt({ x: y * 6, y: -x * 6 }); // Subtle 6 degree max tilt
   };
   ...
   if (!match) return <div className="p-20 text-white font-black text-center uppercase tracking-[0.5em]">Operation_Not_Found</div>;
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-slate-950 via-transparent to-slate-950" />
         ...
         <div className="max-w-[1700px] mx-auto relative z-10 pt-24 px-8">
            <Link href="/coaching/upcoming" className="flex items-center gap-3 text-[9px] font-black text-slate-500 hover:text-white uppercase tracking-[0.4em] mb-12 group transition-all">
               ...
               Operations_Registry // Return_Vantage
            </Link>
            ...
            <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-8">
               <div className="animate-slide-up flex-1 w-full">
                   ...
                   <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 mb-8 pb-8 border-b border-white/5 relative group/header">
                      {/* Vantage_Scan Corner Frame */}
                      <div className="absolute top-0 left-0 w-8 h-8 border-t border-l border-yellow-500/20 group-hover/header:border-yellow-500/40 transition-colors" />
                      
                      <div className="flex flex-col gap-4">
                         <div className="flex items-center gap-3 text-[9px] text-yellow-500 font-black uppercase tracking-[0.5em] mb-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-yellow-500 animate-pulse shadow-[0_0_10px_rgba(234,179,8,0.5)]" />
                            Tactical_Vantage: Active
                         </div>
                     ...
                     <div className="flex gap-10 items-center animate-slide-left">
                        <div className="flex flex-col items-end gap-2 relative">
                           ...
                           <span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.5em] mb-2 border-r-2 border-yellow-500 pr-4">Strategic_Power_Level</span>
                  ...
                  <div className="flex items-center gap-12 text-[9px] font-black text-slate-700 tracking-[0.4em] uppercase border-b border-white/5 pb-4">
                     ...
                     <span className="flex items-center gap-3 text-slate-800"><FaGlobe className="text-yellow-500/30 animate-pulse" /> LIVE_TELEMETER</span>
                  </div>
               </div>
                  ...
                  <div className="flex gap-6 animate-slide-up" style={{ animationDelay: '0.2s' }}>
                     <div className="glass-card hud-border px-8 py-4 bg-slate-950/40 border-green-500/20 relative">
                        ...
                        <div className="text-[8px] text-green-500/80 font-black uppercase tracking-widest mb-2 flex items-center gap-2">
                           <div className="w-1 h-1 rounded-full bg-green-500" /> Personnel_Ready
                        </div>
                     ...
                     {team.some(p => p.status !== 'available') && (
                        <div className="glass-card hud-border px-8 py-4 bg-slate-950/40 border-red-500/20 relative">
                           ...
                           <div className="text-[8px] text-red-500/80 font-black uppercase tracking-widest mb-2 flex items-center gap-2">
                              <div className="w-1 h-1 rounded-full bg-red-500 animate-pulse" /> Constraints
                           </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-24">
               {/* Visual Pitch_Control 2.0 - Maximum Impact */}
                <div className="lg:col-span-8 animate-slide-up" style={{ animationDelay: '0.1s' }}>
                  ...
                  >
                     ...
                     {slots.map((slot) => {
                                        ...
                                        <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 group-hover:scale-110 transition-transform duration-500 z-50">
                                           <div className="relative glass-card bg-slate-950/95 backdrop-blur-xl px-5 py-2 border border-yellow-500/50 shadow-[0_15px_40px_rgba(0,0,0,0.9)] group-hover:border-yellow-500 transition-colors min-w-[160px]">
                                              ...
                                              <div className="flex flex-col items-center">
                                                 <span className="text-[7px] text-yellow-500/60 font-black uppercase tracking-[0.4em] mb-0.5">BIO_SIGNAL: LOCKED</span>
                                                 <span className="text-[11px] md:text-[14px] font-black text-white uppercase tracking-[0.2em] font-mono leading-none truncate w-full text-center">{assignedPlayer.name}</span>
                                              </div>
                                           </div>
                                        </div>
                                     </>
                               ...
                               {isOpen && (
                                 ...
                                 >
                                    <div className="p-5 border-b border-white/5 bg-gradient-to-r from-yellow-500/10 via-transparent to-transparent flex items-center justify-between">
                                       <div>
                                          <p className="text-[11px] font-black text-white uppercase tracking-[0.3em] italic mb-1">Sector: {slot.id}</p>
                                          <div className="flex gap-2 items-center text-[7px] text-slate-500 font-black uppercase tracking-widest">
                                             <FaSatellite className="text-yellow-500/50" /> 
                                       ...
                                       <button onClick={(e) => { e.stopPropagation(); setShowAllPlayers(!showAllPlayers); }} className={`text-[8px] font-black uppercase px-3 py-2 border rounded-sm transition-all ${showAllPlayers ? 'bg-yellow-500 text-slate-950 border-yellow-500' : 'text-slate-500 border-white/10 hover:border-white/30'}`}>
                                           {showAllPlayers ? 'Filtered_Mode' : 'Override_Lock'}
                                        </button>
                                     </div>
                                     <div className="max-h-[350px] overflow-y-auto custom-scrollbar p-3 space-y-2 relative bg-[#050505]/80">
                                        <button onClick={() => updatePosition(slot.id, '')} className="w-full text-left p-4 hover:bg-red-500/10 text-red-500 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center border border-red-500/20 rounded-lg group/withdraw">
                                           <FaTimes className="mr-3 group-hover/withdraw:rotate-90 transition-transform" /> DE-AUTHORIZE PERSON
                                        </button>
                      ...
                      {/* Formation_Oracle 2.0 - Tactical Monitor (Repositioned to Top-Right) */}
                           ...
                           <div className="flex flex-col gap-4">
                              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                                 <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em]">Formation_Oracle</p>
                                 </div>
                                 <span className="text-[7px] text-slate-800 font-mono">SYS_VER: 2.0.4</span>
                              </div>
                  ...
                  {inspectingPlayer && (
                     <div className="glass-card hud-border p-10 bg-slate-950/80 backdrop-blur-xl border-yellow-500/30 animate-fade-in relative">
                        ...
                        <div className="flex justify-between items-center mb-10">
                           <span className="text-[9px] text-yellow-500 font-black uppercase tracking-[0.5em]">Asset_Identity_Sync</span>
                        ...
                        <div className="flex gap-8 mb-10">
                           ...
                           <div>
                              ...
                              <div className="flex flex-col gap-1">
                                 ...
                                 <span className="text-slate-600 font-mono text-[7px] uppercase tracking-widest">Signal: Locked_A-Tier</span>
                              </div>
                           </div>
                        </div>
                   ...
                   <section className="glass-card hud-border p-10 bg-slate-950/40 relative">
                      ...
                      <h3 className="text-[11px] font-black text-yellow-500 uppercase tracking-[0.5em] mb-10 pb-6 border-b border-white/5 flex items-center gap-3 italic">
                         <FaRunning className="text-sm animate-pulse" /> Strategic_Readiness
                      </h3>
                      <div className="space-y-8">
                         <div className="flex justify-between items-center">
                            <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Deployment_Quota</span>
                         ...
                         <div className="flex justify-between items-center text-[8px] font-black uppercase tracking-[0.4em]">
                            <span className={match.homeLineup?.length === 7 ? 'text-green-500 animate-pulse' : 'text-slate-700'}>{match.homeLineup?.length === 7 ? 'GRID_LOCKED_ALPHA' : 'DATA_MISSING_SYNC'}</span>
                            <span className="text-slate-800">Operational_Status // R-9</span>
                         </div>
                      </div>
                   </section>
                    ...
                    <section className="glass-card hud-border p-10 bg-slate-950/40 relative">
                       <div className="absolute top-0 left-0 w-8 h-8 border-t border-l border-white/5" />
                       <h3 className="text-[11px] font-black text-white uppercase tracking-[0.5em] mb-10 pb-6 border-b border-white/5 flex items-center gap-3 italic">
                          <FaUsers className="text-yellow-500/50" /> Reserve_Link // Standby_Assets
                       </h3>
                       <div className="grid grid-cols-1 gap-4 max-h-[300px] overflow-y-auto custom-scrollbar pr-4">
                          {bench.length > 0 ? bench.map((p, i) => (
                             ...
                             <div className="py-10 text-center border border-dashed border-white/5 rounded-lg">
                                <p className="text-[8px] text-slate-700 font-black uppercase tracking-[0.5em]">No_Assets_In_Reserve</p>
                             </div>
                          )}
                       </div>
                    </section>
                   ...
                   <section className="glass-card hud-border p-10 bg-slate-950/40 relative">
                       <div className="absolute top-0 left-0 w-8 h-8 border-t border-l border-white/5" />
                      <h3 className="text-[11px] font-black text-white uppercase tracking-[0.5em] mb-10 pb-6 border-b border-white/5 flex items-center gap-3">
                         <FaMicrochip className="text-yellow-500/50" /> Intelligence_Briefing
                      </h3>
                      <div className="grid grid-cols-2 gap-6">
                         {[
                            { icon: FaLandmark, val: match.venue, label: 'Location' },
                            { icon: FaClock, val: match.time, label: 'Mission_Time' },
                            { icon: FaCloudSun, val: match.weather || 'STABLE', label: 'Atmosphere' },
                            { icon: FaTv, val: 'LIVE_FEED', label: 'Tactical_Link' }
                         ].map((it, i) => (
