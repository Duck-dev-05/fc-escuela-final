...
export default function Home() {
    ...
    const fetchNextMatch = async () => {
      try {
        ...
        const upcomingMatches = allMatches
          .filter((match: Match) => match && match.date && new Date(match.date) > new Date())
  ...
  return (
    <div className="min-h-screen bg-transparent selection:bg-yellow-500/10 overflow-x-hidden">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="container-custom relative z-20 px-4 flex flex-col items-center justify-center">
          <div className="flex flex-col items-center mb-12 relative group cursor-default">
             <h1 className="text-4xl sm:text-7xl md:text-[8rem] lg:text-[10rem] font-black text-center leading-[0.8] tracking-[-0.08em] uppercase italic relative">
                ...
                <span className="block text-slate-900/80 mt-2 tracking-tighter opacity-80 group-hover:opacity-100 transition-all">PORTAL</span>
             </h1>
          </div>

          <div className="flex flex-col sm:flex-row gap-6 md:gap-8 items-center justify-center w-full max-w-lg md:max-w-none px-6">
            <Link href="/matches" className="magnetic-card h-14 md:h-16 w-full sm:w-64 flex items-center justify-center px-8 rounded-xl glass-card border-slate-200 bg-white/50 group relative overflow-hidden hover:border-yellow-500/40 transition-all">
              <span className="relative z-10 text-sm font-black text-slate-500 uppercase tracking-[0.2em] italic group-hover:text-yellow-600 transition-colors flex items-center gap-4 whitespace-nowrap">
                <FaTrophy className="text-sm opacity-50 transition-opacity group-hover:opacity-100" /> Matches
              </span>
              <div className="absolute inset-0 bg-yellow-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            </Link>
            
            <Link href="/ticketing" className="magnetic-card h-14 md:h-16 w-full sm:w-64 flex items-center justify-center px-8 rounded-xl bg-yellow-500 border border-yellow-500 group relative overflow-hidden shadow-[0_10px_40px_rgba(234,179,8,0.2)]">
              <span className="relative z-10 text-sm font-black text-slate-950 uppercase tracking-[0.2em] italic group-hover:text-white transition-colors flex items-center gap-4 whitespace-nowrap">
                Tickets <FaTicketAlt className="text-sm transition-transform group-hover:scale-110" />
              </span>
      ...
      <section className="py-32 md:py-48 relative border-t border-slate-100 bg-slate-50/50 backdrop-blur-sm">
        <div className="container-custom px-6 md:px-12">
          
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-stretch">
            {/* Spotlight Match Intel: Symmetric Center */}
            <div className="glass-card hud-border p-8 md:p-12 rounded-xl bg-white/80 relative overflow-hidden group flex flex-col items-center justify-center text-center">
               ...
               <div className="pt-8 w-full">
                  <span className="text-yellow-600/60 font-black text-[10px] uppercase tracking-[0.4em] mb-6 block">NEXT_DEPLOYMENT</span>
                  ...
                  <div className="flex flex-col items-center gap-3 mb-10">
                     <div className="text-3xl font-black text-yellow-600 italic tracking-[0.2em]">{nextMatch?.time || '20:00'}</div>
                     <div className="py-2 px-6 glass-card border-yellow-500/20 bg-yellow-500/5 rounded-lg">
                        <span className="text-[10px] font-black text-yellow-600 uppercase tracking-widest">{nextMatch?.venue || 'NEUTRAL_TERRITORY'}</span>
                     </div>
                  </div>
                  <Link href="/ticketing" className="w-full py-5 bg-slate-50 border border-slate-100 hover:border-yellow-500/40 hover:bg-yellow-500 hover:text-slate-950 flex items-center justify-center gap-4 transition-all duration-500 rounded-lg group/btn">
            ...
            {/* Matrix Sidecard: Symmetric Center */}
            <div className="glass-card hud-border rounded-xl bg-white/80 p-10 md:p-12 flex flex-col items-center justify-between text-center group overflow-hidden relative">
               ...
               <div className="pt-12 flex flex-col items-center">
                  <FaUsers className="text-4xl text-yellow-600/40 mb-10 group-hover:scale-110 transition-transform" />
                  <h3 className="text-5xl md:text-6xl font-black text-slate-900 italic uppercase tracking-tighter leading-none mb-8">Vanguard <span className="text-yellow-600 block mt-2">Registry</span></h3>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-[0.2em] leading-relaxed mb-12 max-w-sm">Authorized operators only. Deploying legacy protocols for the next tactical season.</p>
               </div>
               <Link href="/auth/register" className="w-full flex items-center justify-center gap-6 p-5 glass-card border-slate-200 bg-slate-50 rounded-lg hover:border-yellow-500/40 hover:bg-yellow-500 hover:text-slate-950 transition-all group/btn">
                  <span className="text-[10px] font-black uppercase tracking-[0.4em]">Initialize Registry Access</span>
        ...
        <div className="container-custom relative z-10 text-center px-6">
          <div className="max-w-5xl mx-auto flex flex-col items-center">
             ...
             <div className="flex flex-col sm:flex-row gap-10 items-center justify-center">
                <Link href="/auth/register" className="py-8 px-24 bg-yellow-500 text-slate-950 text-xl font-black uppercase tracking-widest italic rounded-xl shadow-[0_20px_60px_rgba(234,179,8,0.2)] hover:scale-110 transition-transform">Enroll Now</Link>
